...
def create_model(num_users, num_restaurants, num_columns_details, learning_rate, emb_size, dropout):
    ...
    # plot_model(final_model, to_file="baseline_model.png", show_shapes=True)

    plot_model(final_model)
    return final_model


def train_model(final_model, x_train_data,y_train_data, x_val_data, y_val_data, batch_size, num_epochs, patience):
    ...
    history = final_model.fit(x=[x_train_data['userId'], x_train_data['restaurantId']],
                              y=y_train_data,
                              validation_data=([x_val_data['userId'], x_val_data['restaurantId']], y_val_data),
...
def plot_history(history, attribute, message):
    ...
    plt.savefig('./plots/baseline_' + message)
    # plt.show()
...
def plot_dual_history(history):
    ...
    plt.savefig('./plots/baseline_Model_Losses')
    # plt.show()


def evaluate_model(final_model, full_data, x_data, y_data):
    test_loss, test_auc, test_acc = final_model.evaluate([x_data['userId'], x_data['restaurantId']],
                         y_data,
                         steps=len(full_data),
                         verbose=1)
...
def predict_model(final_model, x_test_data):
    # Obtenemos las predicciones para todos los ejemplos del conjunto de test
    predictions = final_model.predict([x_test_data['userId'], x_test_data['restaurantId']],
...
if __name__ == '__main__':
    ...
    final_model, history = train_model(final_model=final_model, x_train_data=x_train, y_train_data=y_train,
                                       x_val_data=x_val, y_val_data=y_val, batch_size=BATCH_SIZE, num_epochs=NUM_EPOCHS,
                                       patience=PATIENCE)
    # end time
    end = time.time()
    ...
    print("Predecimos con mejor modelo los datos en conjunto de test")
    predictions = predict_model(final_model=final_model, x_test_data=x_test)

    check_predictions(test_data=test_data, pred=predictions)

    # Evaluar el modelo en el conjunto de test
    print("Evaluamos mejor modelo en conjunto de test")
    evaluate_model(final_model=final_model, full_data=test_data, x_data=x_test, y_data=y_test)

    # Evaluar el modelo en el conjunto de val
    print("Evaluamos mejor modelo en conjunto de validación")
    evaluate_model(final_model=final_model, full_data=val_data, x_data=x_val, y_data=y_val)
